...
void LightTree::makeTree(edm::Service<TFileService> & aFile,
  ...
  outputTree->Branch("lc_seedu", &lc_seedu);
  outputTree->Branch("lc_seedv", &lc_seedv);
  ...
  outputTree->Branch("lc_isHD", &lc_isHD);
			   ...
			   const std::vector<std::vector<double> > & newSimLCmult)
{
  ...
  lc_x.reserve(nLC);
  lc_y.reserve(nLC);
  lc_seedu.reserve(nLC);
  lc_seedv.reserve(nLC);
  lc_z.reserve(nLC);
  lc_algo.reserve(nLC);
